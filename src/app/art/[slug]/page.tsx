...
export default function ArtworkDetailPage() {
  ...
  const artwork = useMemo(
    () => artworks.find((item) => item.slug === slug),
    [slug],
  );
    ...
    return (
      <div className="site-container page-shell flex min-h-[60vh] flex-col items-center justify-center text-center">
        ...
        <p className="body-text mt-4 max-w-md">
          The piece you are looking for may have been moved or is no longer part
          of the collection.
        </p>
  ...
  return <ArtworkDetailContent key={artwork.slug} artwork={artwork} />;
}
...
function ArtworkDetailContent({ artwork }: { artwork: Artwork }) {
  ...
  const dispatchNote = artwork.dispatchNote ?? config.defaultDispatchNote;
  const careGuide = artwork.careGuide ?? config.defaultCareGuide;
  const shippingReturns =
    artwork.shippingReturns ?? config.defaultShippingReturns;
  const beforeYouBuy = artwork.beforeYouBuy ?? config.defaultBeforeYouBuy;
